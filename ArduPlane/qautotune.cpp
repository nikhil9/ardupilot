...
bool QuadPlane::qautotune_init(bool ignore_checks)
{
    bool success = true;
    ...
    switch (mode) {
        ...
        case TUNING:
            ...
            if (success) {
            	...
            	qautotune_load_intra_test_gains();
...
const char *QuadPlane::qautotune_level_issue_string() const
{
...
void QuadPlane::qautotune_send_step_string()
{
    if (pilot_override) {
    ...
    case WAITING_FOR_LEVEL:
        gcs().send_text(MAV_SEVERITY_INFO, "AutoTune: WFL (%s) (%f > %f)", qautotune_level_issue_string(), (double)(level_problem.current*0.01f), (double)(level_problem.maximum*0.01f));
        return;
    case UPDATE_GAINS:
...
const char *QuadPlane::qautotune_type_string() const
{
    switch (tune_type) {
    case RD_UP:
        return "Rate D Up";
...
void QuadPlane::qautotune_do_gcs_announcements()
{
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "AutoTune: (%c) %s", axis_char, qautotune_type_string());
    qautotune_send_step_string();
...
//    target_yaw_rate = get_pilot_desired_yaw_rate(channel_yaw->get_control_in());
    target_yaw_rate = get_desired_yaw_rate_cds();

    // get pilot desired climb rate
//    target_climb_rate = get_pilot_desired_climb_rate(channel_throttle->get_control_in());
    target_climb_rate = get_pilot_desired_climb_rate_cms();
...
//    }else{
        ...
        if (zero_rp_input) {
            ...
            qautotune_get_poshold_attitude(target_roll, target_pitch, desired_yaw);
        }
        ...
        if (pilot_override || mode != TUNING) {
            ...
            qautotune_attitude_control();
            // tell the user what's going on
            qautotune_do_gcs_announcements();
        }
...
bool QuadPlane::qautotune_check_level(const LEVEL_ISSUE issue, const float current, const float maximum)
{
    if (current > maximum) {
...
void QuadPlane::qautotune_attitude_control()
{
    ...
    case WAITING_FOR_LEVEL:
        ...
        qautotune_get_poshold_attitude(qautotune_roll_cd, qautotune_pitch_cd, desired_yaw);

        // hold level attitude
        attitude_control->input_euler_angle_roll_pitch_yaw(qautotune_roll_cd, qautotune_pitch_cd, desired_yaw, true);

        // hold the copter level for 0.5 seconds before we begin a twitch
        // reset counter if we are no longer level
        if (!qautotune_currently_level()) {
    ...
    case TWITCHING:
        ...
        case RD_UP:
        case RD_DOWN:
        	qautotune_twitching_test_rate(rotation_rate, target_rate, test_rate_min, test_rate_max);
        	qautotune_twitching_measure_acceleration(test_accel_max, rotation_rate, rate_max);
            if (lean_angle >= target_angle) {
                step = UPDATE_GAINS;
            }
            break;
        case RP_UP:
        	qautotune_twitching_test_rate(rotation_rate, target_rate*(1+0.5f*autotune_aggressiveness), test_rate_min, test_rate_max);
        	qautotune_twitching_measure_acceleration(test_accel_max, rotation_rate, rate_max);
            if (lean_angle >= target_angle) {
                step = UPDATE_GAINS;
            }
        ...
        case SP_DOWN:
        case SP_UP:
        	qautotune_twitching_test_angle(lean_angle, rotation_rate, target_angle*(1+0.5f*autotune_aggressiveness), test_angle_min, test_angle_max, test_rate_min, test_rate_max);
        	qautotune_twitching_measure_acceleration(test_accel_max, rotation_rate - direction_sign * start_rate, rate_max);
            break;
        }

        // log this iterations lean angle and rotation rate
        Log_Write_AutoTuneDetails(lean_angle, rotation_rate);
        plane.DataFlash.Log_Write_Rate(ahrs, *motors, *attitude_control, *pos_control);
    ...
    case UPDATE_GAINS:
        ...
        case RD_UP:
            switch (axis) {
            case ROLL:
                qautotune_updating_rate_d_up(tune_roll_rd, autotune_min_d, AUTOTUNE_RD_MAX, AUTOTUNE_RD_STEP, tune_roll_rp, AUTOTUNE_RP_MIN, AUTOTUNE_RP_MAX, AUTOTUNE_RP_STEP, target_rate, test_rate_min, test_rate_max);
                break;
            case PITCH:
            	qautotune_updating_rate_d_up(tune_pitch_rd, autotune_min_d, AUTOTUNE_RD_MAX, AUTOTUNE_RD_STEP, tune_pitch_rp, AUTOTUNE_RP_MIN, AUTOTUNE_RP_MAX, AUTOTUNE_RP_STEP, target_rate, test_rate_min, test_rate_max);
                break;
            case YAW:
            	qautotune_updating_rate_d_up(tune_yaw_rLPF, AUTOTUNE_RLPF_MIN, AUTOTUNE_RLPF_MAX, AUTOTUNE_RD_STEP, tune_yaw_rp, AUTOTUNE_RP_MIN, AUTOTUNE_RP_MAX, AUTOTUNE_RP_STEP, target_rate, test_rate_min, test_rate_max);
                break;
            }
            break;
        ...
        case RD_DOWN:
            switch (axis) {
            case ROLL:
            	qautotune_updating_rate_d_down(tune_roll_rd, autotune_min_d, AUTOTUNE_RD_STEP, tune_roll_rp, AUTOTUNE_RP_MIN, AUTOTUNE_RP_MAX, AUTOTUNE_RP_STEP, target_rate, test_rate_min, test_rate_max);
                break;
            case PITCH:
            	qautotune_updating_rate_d_down(tune_pitch_rd, autotune_min_d, AUTOTUNE_RD_STEP, tune_pitch_rp, AUTOTUNE_RP_MIN, AUTOTUNE_RP_MAX, AUTOTUNE_RP_STEP, target_rate, test_rate_min, test_rate_max);
                break;
            case YAW:
            	qautotune_updating_rate_d_down(tune_yaw_rLPF, AUTOTUNE_RLPF_MIN, AUTOTUNE_RD_STEP, tune_yaw_rp, AUTOTUNE_RP_MIN, AUTOTUNE_RP_MAX, AUTOTUNE_RP_STEP, target_rate, test_rate_min, test_rate_max);
                break;
            }
            break;
        ...
        case RP_UP:
            switch (axis) {
            case ROLL:
            	qautotune_updating_rate_p_up_d_down(tune_roll_rd, autotune_min_d, AUTOTUNE_RD_STEP, tune_roll_rp, AUTOTUNE_RP_MIN, AUTOTUNE_RP_MAX, AUTOTUNE_RP_STEP, target_rate, test_rate_min, test_rate_max);
                break;
            case PITCH:
            	qautotune_updating_rate_p_up_d_down(tune_pitch_rd, autotune_min_d, AUTOTUNE_RD_STEP, tune_pitch_rp, AUTOTUNE_RP_MIN, AUTOTUNE_RP_MAX, AUTOTUNE_RP_STEP, target_rate, test_rate_min, test_rate_max);
                break;
            case YAW:
            	qautotune_updating_rate_p_up_d_down(tune_yaw_rLPF, AUTOTUNE_RLPF_MIN, AUTOTUNE_RD_STEP, tune_yaw_rp, AUTOTUNE_RP_MIN, AUTOTUNE_RP_MAX, AUTOTUNE_RP_STEP, target_rate, test_rate_min, test_rate_max);
                break;
            }
            break;
        ...
        case SP_DOWN:
            switch (axis) {
            case ROLL:
            	qautotune_updating_angle_p_down(tune_roll_sp, AUTOTUNE_SP_MIN, AUTOTUNE_SP_STEP, target_angle, test_angle_max, test_rate_min, test_rate_max);
                break;
            case PITCH:
            	qautotune_updating_angle_p_down(tune_pitch_sp, AUTOTUNE_SP_MIN, AUTOTUNE_SP_STEP, target_angle, test_angle_max, test_rate_min, test_rate_max);
                break;
            case YAW:
            	qautotune_updating_angle_p_down(tune_yaw_sp, AUTOTUNE_SP_MIN, AUTOTUNE_SP_STEP, target_angle, test_angle_max, test_rate_min, test_rate_max);
                break;
            }
            break;
        // Check results after mini-step to increase stabilize P gain
        case SP_UP:
            switch (axis) {
            case ROLL:
            	qautotune_updating_angle_p_up(tune_roll_sp, AUTOTUNE_SP_MAX, AUTOTUNE_SP_STEP, target_angle, test_angle_max, test_rate_min, test_rate_max);
                break;
            case PITCH:
            	qautotune_updating_angle_p_up(tune_pitch_sp, AUTOTUNE_SP_MAX, AUTOTUNE_SP_STEP, target_angle, test_angle_max, test_rate_min, test_rate_max);
                break;
            case YAW:
            	qautotune_updating_angle_p_up(tune_yaw_sp, AUTOTUNE_SP_MAX, AUTOTUNE_SP_STEP, target_angle, test_angle_max, test_rate_min, test_rate_max);
                break;
            }
            break;
        }
        ...
        if (counter >= AUTOTUNE_SUCCESS_COUNT) {
            ...
            case RD_DOWN:
                ...
                switch (axis) {
                case ROLL:
                    tune_roll_rd = MAX(autotune_min_d, tune_roll_rd * AUTOTUNE_RD_BACKOFF);
                    tune_roll_rp = MAX(AUTOTUNE_RP_MIN, tune_roll_rp * AUTOTUNE_RD_BACKOFF);
                    break;
                case PITCH:
                    tune_pitch_rd = MAX(autotune_min_d, tune_pitch_rd * AUTOTUNE_RD_BACKOFF);
            ...
            case SP_UP:
                ...
                case ROLL:
                    ...
                    if (qautotune_pitch_enabled()) {
                        axis = PITCH;
                    } else if (qautotune_yaw_enabled()) {
                        axis = YAW;
                    } else {
                        complete = true;
                    }
                ...
                if (complete) {
                    mode = SUCCESS;
                    qautotune_update_gcs(AUTOTUNE_MESSAGE_SUCCESS);
...
void QuadPlane::qautotune_backup_gains_and_initialise()
{
    // initialise state because this is our first time
    if (qautotune_roll_enabled()) {
        axis = ROLL;
    } else if (qautotune_pitch_enabled()) {
        axis = PITCH;
    } else if (qautotune_yaw_enabled()) {
        axis = YAW;
    }
...
void QuadPlane::qautotune_load_orig_gains()
{
...
void QuadPlane::qautotune_load_tuned_gains()
{
...
void QuadPlane::qautotune_load_intra_test_gains()
{
...
void QuadPlane::qautotune_load_twitch_gains()
{
    switch (axis) {
        case ROLL:
            attitude_control->get_rate_roll_pid().kP(tune_roll_rp);
...
void QuadPlane::qautotune_save_tuning_gains()
{
    ...
    if (mode == SUCCESS) {
        ...
        if (qautotune_roll_enabled() && !is_zero(tune_roll_rp)) {
            // rate roll gains
            attitude_control->get_rate_roll_pid().kP(tune_roll_rp);
            attitude_control->get_rate_roll_pid().kI(tune_roll_rp*AUTOTUNE_PI_RATIO_FINAL);
            attitude_control->get_rate_roll_pid().kD(tune_roll_rd);
        ...
        if (qautotune_pitch_enabled() && !is_zero(tune_pitch_rp)) {
            // rate pitch gains
            attitude_control->get_rate_pitch_pid().kP(tune_pitch_rp);
            attitude_control->get_rate_pitch_pid().kI(tune_pitch_rp*AUTOTUNE_PI_RATIO_FINAL);
            attitude_control->get_rate_pitch_pid().kD(tune_pitch_rd);
        ...
        if (qautotune_yaw_enabled() && !is_zero(tune_yaw_rp)) {
            // rate yaw gains
            attitude_control->get_rate_yaw_pid().kP(tune_yaw_rp);
            attitude_control->get_rate_yaw_pid().kI(tune_yaw_rp*AUTOTUNE_YAW_PI_RATIO_FINAL);
            attitude_control->get_rate_yaw_pid().kD(0.0f);
        ...
        qautotune_update_gcs(AUTOTUNE_MESSAGE_SAVED_GAINS);
...
void QuadPlane::qautotune_update_gcs(uint8_t message_id)
{
    switch (message_id) {
...
inline bool QuadPlane::qautotune_roll_enabled() {
    return autotune_axis_bitmask & AUTOTUNE_AXIS_BITMASK_ROLL;
}

inline bool QuadPlane::qautotune_pitch_enabled() {
    return autotune_axis_bitmask & AUTOTUNE_AXIS_BITMASK_PITCH;
}

inline bool QuadPlane::qautotune_yaw_enabled() {
    return autotune_axis_bitmask & AUTOTUNE_AXIS_BITMASK_YAW;
}

// twitching_test_rate - twitching tests
// update min and max and test for end conditions
void QuadPlane::qautotune_twitching_test_rate(float rate, float rate_target_max, float &meas_rate_min, float &meas_rate_max)
{
    ...
    if (meas_rate_max-meas_rate_min > meas_rate_max*autotune_aggressiveness) {
...
// update min and max and test for end conditions
void QuadPlane::qautotune_twitching_test_angle(float angle, float rate, float angle_target_max, float &meas_angle_min, float &meas_angle_max, float &meas_rate_min, float &meas_rate_max)
{
    ...
    if (meas_angle_max-meas_angle_min > meas_angle_max*autotune_aggressiveness) {
...
void QuadPlane::qautotune_twitching_measure_acceleration(float &rate_of_change, float rate_measurement, float &rate_measurement_max)
{
...
void QuadPlane::qautotune_updating_rate_d_up(float &tune_d, float tune_d_min, float tune_d_max, float tune_d_step_ratio, float &tune_p, float tune_p_min, float tune_p_max, float tune_p_step_ratio, float rate_target, float meas_rate_min, float meas_rate_max)
{
...
// optimize D term while keeping the maximum just below the target by adjusting P
void QuadPlane::qautotune_updating_rate_d_down(float &tune_d, float tune_d_min, float tune_d_step_ratio, float &tune_p, float tune_p_min, float tune_p_max, float tune_p_step_ratio, float rate_target, float meas_rate_min, float meas_rate_max)
{
    if (meas_rate_max > rate_target) {
        // if maximum measurement was higher than target
        // reduce P gain (which should reduce maximum)
...
void QuadPlane::qautotune_updating_rate_p_up_d_down(float &tune_d, float tune_d_min, float tune_d_step_ratio, float &tune_p, float tune_p_min, float tune_p_max, float tune_p_step_ratio, float rate_target, float meas_rate_min, float meas_rate_max)
{
    if (meas_rate_max > rate_target*(1+0.5f*autotune_aggressiveness)) {
        // ignore the next result unless it is the same as this one
        ignore_next = true;
        // if maximum measurement was greater than target so increment the success counter
        counter++;
    } else if ((meas_rate_max < rate_target) && (meas_rate_max > rate_target*(1.0f-AUTOTUNE_D_UP_DOWN_MARGIN)) && (meas_rate_max-meas_rate_min > meas_rate_max*autotune_aggressiveness) && (tune_d > tune_d_min)) {
...
void QuadPlane::qautotune_updating_angle_p_down(float &tune_p, float tune_p_min, float tune_p_step_ratio, float angle_target, float meas_angle_max, float meas_rate_min, float meas_rate_max)
{
    if (meas_angle_max < angle_target*(1+0.5f*autotune_aggressiveness)) {
        if (ignore_next == false) {
...
void QuadPlane::qautotune_updating_angle_p_up(float &tune_p, float tune_p_max, float tune_p_step_ratio, float angle_target, float meas_angle_max, float meas_rate_min, float meas_rate_max)
{
    if ((meas_angle_max > angle_target*(1+0.5f*autotune_aggressiveness)) ||
            ((meas_angle_max > angle_target) && (meas_rate_min < -meas_rate_max*autotune_aggressiveness))) {
        // ignore the next result unless it is the same as this one
        ignore_next = true;
        // if maximum measurement was greater than target so increment the success counter
        counter++;
...
void QuadPlane::qautotune_get_poshold_attitude(float &roll_cd_out, float &pitch_cd_out, float &yaw_cd_out)
{
